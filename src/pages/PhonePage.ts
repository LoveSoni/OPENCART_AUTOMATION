...
export class PhonePage extends BasePage {
  ...
  constructor(page: Page, baseUrl: string) {
    ...
    // Initialize locators
    this.pageTitle = this.page.locator('h2:has-text("Phones & PDAs")');
  ...
  async isPhonePageDisplayed(): Promise<boolean> {
    try {
      await this.waitForElement(this.pageTitle, 10000);
      const titleText = await this.pageTitle.textContent();
      return titleText ? titleText.includes('Phones & PDAs') : false;
    } catch (error) {
      return false;
    }
  }
  ...
  async getAllProductNames(): Promise<string[]> {
    try {
      console.log('Starting to extract product names...');
      await this.waitForElement(this.productItems.first(), 10000);
  ...
  async getAllProductPricesFromAllPages(): Promise<{ name: string; price: string; originalPrice?: string }[]> {
    ...
    try {
      ...
      do {
        console.log(`Extracting prices on page ${currentPageNum}...`);
